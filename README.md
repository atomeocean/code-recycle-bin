...
以本仓库为模板，快速初始化一个基于 [VitePress](https://vitepress.dev/) 的开源文档站。两个基于它演进的项目（[logbook](https://github.com/atomeocean/logbook)、Internal）上已验证的通用能力都沉淀到了这里。

**技术栈：** VitePress (Vue 3) + TypeScript + Sass + Node.js 24
...
- 🧩 **路径别名**：`@ao-components`、`@ao-utils` 等语义化别名，`alias.ts` 与 `tsconfig.json` 保持一致。
...
```shell
nvm use            # 切换到 Node 24
npm install        # 安装依赖（会通过 prepare 自动初始化 husky）
...
- 增删语言、修改导航栏与「编辑本页」链接，都在 [`docs/.vitepress/config/i18n.config.ts`](docs/.vitepress/config/i18n.config.ts)。记得把其中的 `<YOUR_REPO>` 替换为实际仓库名。
